...
### 🟡 BP-06 — `ACPServer` (legacy TCP) разделяет `ACPProtocol` между клиентами

**Файл:** `src/codelab/server/server.py`

```python
self.protocol = ACPProtocol()  # один экземпляр на всех клиентов
```

Несколько одновременных TCP-клиентов будут делить `_sessions`, `_authenticated` и `_runtime_capabilities`. Это race condition и нарушение изоляции. Если `server.py` нужен для тестов или совместимости — каждый клиент должен получать свой экземпляр `ACPProtocol`.
...
- Конкурентного доступа нескольких соединений к разделяемому `ACPProtocol` в `server.py`.
...
| 1.6 | Исправить shared `ACPProtocol` в legacy TCP сервере | `server.py` | 2 ч |
