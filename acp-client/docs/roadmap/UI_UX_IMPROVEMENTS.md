...
2. [Анализ конкурентов](#анализ-конкурентов)
...
- **Windsurf**: Multi-pane layout с динамическим переразмешением
...
- Переразмешение панелей (drag-n-drop)
...
1. Создать UIStateViewModel и UIStateObservable
2. Рефакторить Sidebar для табов
3. Добавить HelpSystem и HelpViewModel
4. Реализовать CommandPalette
5. Добавить статусные индикаторы
6. Обновить все ViewModels для новых состояний
...
1. **Неделя 1**: Approve Фаза 1 требования
2. **Неделя 2-3**: Разработка Фазы 1
3. **Неделя 4**: Release Фазы 1, получить feedback
4. **После**: Планировать Фазу 2 на основе feedback
