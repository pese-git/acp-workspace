...
6. [Dependency Injection (управление зависимостями)](#dependency-injection)
7. [Presentation Layer (слой представления)](#presentation-layer)
8. [TUI Layer (слой пользовательского интерфейса)](#tui-layer)
9. [Правила взаимодействия между слоями](#правила-взаимодействия)
10. [Примеры типичных сценариев](#примеры-типичных-сценариев)
...
└──────────────────────────▲────────────────────────────────────┬─┘
                           │                                    │
              используют Repository & Service интерфейсы       │
                           │                                    │
┌──────────────────────────┴────────────────────────────────────▼─┐
...
Dependency Injection (DI) — это паттерн инверсии управления (IoC), который позволяет:

- **Слабую связанность** между компонентами (компоненты зависят от интерфейсов, а не реализаций)
- **Гибкость** в выборе реализаций (легко менять WebSocketTransport на TCPTransport)
- **Тестируемость** (можно подставлять моки вместо реальных объектов)
- **Управление лайфциклом** компонентов (singleton, transient, scoped)

### DIContainer

**Файл:** [`acp-client/src/acp_client/infrastructure/di_container.py`](../../src/acp_client/infrastructure/di_container.py)

Контейнер для управления регистрацией и разрешением зависимостей:
...
class Scope(Enum):
    ...
    TRANSIENT = "transient"  # Новый экземпляр при каждом разрешении
    SCOPED = "scoped"        # Один экземпляр на область/сессию
...
class DIContainer:
    ...
    def register(
        ...
        """Регистрировать интерфейс с реализацией.
        
        Args:
            interface: Интерфейс (ABC или Protocol)
            implementation: Реализация (класс или factory функция)
            scope: Жизненный цикл компонента
    ...
    def resolve(self, interface: type[T]) -> T:
        """Разрешить зависимость (получить экземпляр).
        ...
        Пример:
            >>> transport = container.resolve(TransportService)
            >>> # Автоматически создаёт WebSocketTransport если нужен
        """
        ...
    
    def set_instance(self, interface: type[T], instance: T) -> None:
        """Зарегистрировать готовый экземпляр (для singleton).
        
        Пример:
            >>> nav_manager = NavigationManager(app)
            >>> container.set_instance(NavigationManager, nav_manager)
        """
        ...
```
...
```python
class DIBootstrapper:
    """Инициализирует и настраивает DI контейнер для приложения."""
    
    @staticmethod
    def bootstrap() -> DIContainer:
        """Создает и настраивает DI контейнер с регистрацией всех компонентов.
        
        Регистрирует:
        - Repository интерфейсы и реализации
        - Service интерфейсы и реализации
        - Use Cases со всеми зависимостями
        - Event Bus
        - Navigation Manager
        - View Models
        
        Returns:
            Полностью настроенный DIContainer
        """
        container = DIContainer()
        
        # 1. Регистрируем Repository (Infrastructure слой)
        container.register(
            SessionRepository,
            InMemorySessionRepository,
            Scope.SINGLETON
        ...
        # 2. Регистрируем Services (Infrastructure слой)
        container.register(
            TransportService,
            WebSocketTransport,
            Scope.SINGLETON
        ...
        # 3. Регистрируем Event Bus (Infrastructure слой)
        container.register(EventBus, EventBus, Scope.SINGLETON)
        
        # 4. Регистрируем Use Cases (Application слой)
        # Use Cases зависят от Repository и Service
        container.register(
            CreateSessionUseCase,
            lambda: CreateSessionUseCase(
        ...
        # 5. Регистрируем ViewModels (Presentation слой)
        container.register(
            ChatViewModel,
            lambda: ChatViewModel(
                container.resolve(SendPromptUseCase),
...
✅ **Можно:**
- Инъектировать интерфейсы (ABC, Protocol) в компоненты
- Создавать несколько реализаций одного интерфейса
- Использовать factory функции для сложного создания
- Регистрировать готовые экземпляры (для singleton)
- Использовать разные scopes для разных компонентов

❌ **Нельзя:**
- Создавать объекты напрямую (use new) без DI
- Зависеть от конкретных реализаций (вместо интерфейсов)
- Регистрировать UI компоненты в контейнере (они создаются Textual)
...
class ACPClientApp(App):
    def __init__(self, host: str, port: int) -> None:
        ...
        self._container = DIBootstrapper.bootstrap()
        
        # Разрешаем компоненты
        self._chat_vm = self._container.resolve(ChatViewModel)
        self._session_vm = self._container.resolve(SessionViewModel)
        self._transport = self._container.resolve(TransportService)

async def on_mount(self) -> None:
    # Компоненты готовы к использованию с правильными зависимостями
    self._chat_view.view_model = self._chat_vm
```
