...
def test_terminal_log_modal_backward_compatibility_add_log(
    terminal_log_view_model: TerminalLogViewModel,
) -> None:
    """Проверить обратную совместимость метода add_log.
    
    Старый метод add_log должен делегировать вызов методу ViewModel.
    """
    modal = TerminalLogModal(
        terminal_log_vm=terminal_log_view_model,
        title="Test Terminal Log",
    ...
    entry = "Legacy log entry"
    
    # Используем старый API через модальное окно
    modal.add_log(entry)
    
    # Проверяем что запись добавлена через ViewModel
    assert len(terminal_log_view_model.log_entries.value) == 1
    assert terminal_log_view_model.log_entries.value[0] == entry


def test_terminal_log_modal_backward_compatibility_set_logs(
    terminal_log_view_model: TerminalLogViewModel,
) -> None:
    """Проверить обратную совместимость метода set_logs.
    
    Старый метод set_logs должен делегировать вызов методу ViewModel.
    """
    modal = TerminalLogModal(
        terminal_log_vm=terminal_log_view_model,
        title="Test Terminal Log",
    ...
    # Добавляем начальные записи через старый API
    modal.add_log("Entry 1")
    modal.add_log("Entry 2")
    
    assert len(terminal_log_view_model.log_entries.value) == 2
    
    # Используем старый API
    new_entries = ["Legacy Entry A", "Legacy Entry B"]
    modal.set_logs(new_entries)
    
    # Проверяем что записи заменены через ViewModel
    assert len(terminal_log_view_model.log_entries.value) == 2
    assert terminal_log_view_model.log_entries.value == new_entries


def test_terminal_log_modal_backward_compatibility_clear_logs(
    terminal_log_view_model: TerminalLogViewModel,
) -> None:
    """Проверить обратную совместимость метода clear_logs.
    
    Старый метод clear_logs должен делегировать вызов методу ViewModel.
    """
    modal = TerminalLogModal(
        terminal_log_vm=terminal_log_view_model,
        title="Test Terminal Log",
    ...
    # Добавляем записи через старый API
    modal.add_log("Entry 1")
    modal.add_log("Entry 2")
    modal.add_log("Entry 3")
    ...
    # Проверяем что записи очищены через ViewModel
    assert len(terminal_log_view_model.log_entries.value) == 0
    assert terminal_log_view_model.log_entries.value == []


def test_terminal_log_modal_backward_compatibility_show_modal(
    terminal_log_view_model: TerminalLogViewModel,
) -> None:
    """Проверить обратную совместимость метода show_modal.
    
    Старый метод show_modal должен делегировать вызов методу ViewModel.
    """
    modal = TerminalLogModal(
        terminal_log_vm=terminal_log_view_model,
        title="Test Terminal Log",
    ...
    # Проверяем что окно показано через ViewModel
    assert terminal_log_view_model.is_visible.value is True
