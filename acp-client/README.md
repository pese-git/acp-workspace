...
Горячие клавиши TUI: `Ctrl+S/Ctrl+B` фокус сессий (дальше `Up/Down + Enter`), `Tab` циклическое переключение фокуса между сессиями и prompt, `Ctrl+Enter` отправить prompt, `Ctrl+R` повторить последнюю неуспешную операцию из очереди retry, `Up/Down` (и `Ctrl+Up/Down`) история prompt в активной сессии, `Ctrl+N` новая сессия, `Ctrl+J/K` переключение сессий, `Ctrl+L` очистить чат, `Ctrl+H` показать встроенную подсказку, `Ctrl+T` открыть полный terminal output последнего tool call, `Ctrl+C` cancel, `Ctrl+Q` выход. При выборе сессии клиент загружает replay-обновления и дорисовывает текстовую историю.

В правой панели TUI отображаются события `tool_call` и `tool_call_update` с текущими статусами вызовов инструментов.
Под историей чата отображается панель `plan` с актуальным snapshot шагов выполнения (`pending`/`in_progress`/`completed`).
В левой колонке под списком сессий отображается дерево файлов текущей рабочей директории активной сессии (скрытые dot-файлы не показываются).
Выбор файла в дереве открывает модальное окно просмотра с подсветкой синтаксиса и номерами строк.
...
Для tool kind сохраняется persistent policy при выборе `allow_always`/`reject_always`; последующие запросы того же kind могут авто-применяться без показа модального окна.
Если модальное окно разрешения не подтверждено за разумное время, запрос завершается по timeout и агент получает отказ выполнения.
...
Replay-история `session/update` также кэшируется локально и используется как fallback, если сервер вернул пустой replay при загрузке сессии.
При расхождении server replay и локального кэша применяется merge с dedupe, чтобы не терять локально накопленные updates.
Параметры запуска TUI (host/port/theme) сохраняются в `~/.acp-client/tui_config.json` и используются как defaults для последующих запусков.
...
Клиент автоматически инициализирует директорию `~/.acp-client` и сохраняет логи в `~/.acp-client/logs/acp-client.log`.

### Использование

Логирование включено по умолчанию и сохраняется в файл автоматически:

```bash
# По умолчанию логи сохраняются в ~/.acp-client/logs/acp-client.log
uv run acp-client --host 127.0.0.1 --port 8080 --tui

# Настройка уровня логирования
uv run acp-client --host 127.0.0.1 --port 8080 --tui --log-level DEBUG

# JSON формат для production окружения
uv run acp-client --host 127.0.0.1 --port 8080 --tui --log-json

# Кастомный путь к файлу логов
uv run acp-client --host 127.0.0.1 --port 8080 --tui --log-file /path/to/logs/custom.log
```
...
### Директория ~/.acp-client

Клиент автоматически создает и использует директорию `~/.acp-client` для хранения:

- `logs/acp-client.log` — логи клиента с ротацией (до 50 MB всего)
- `tui_config.json` — сохраняемые параметры TUI (хост, порт, тема)
- Кэш состояния и другие пользовательские данные

Пример структуры:
```
~/.acp-client/
├── logs/
│   └── acp-client.log      # Основной файл логов
│   └── acp-client.log.1    # Архивные логи (при ротации)
└── tui_config.json         # Конфигурация TUI
```
