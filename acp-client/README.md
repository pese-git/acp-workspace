...
Горячие клавиши TUI: `Ctrl+B` фокус сессий (дальше `Up/Down + Enter`), `Ctrl+Enter` отправить prompt, `Ctrl+R` повторить последний неуспешный prompt, `Ctrl+Up/Down` история prompt в активной сессии, `Ctrl+N` новая сессия, `Ctrl+J/K` переключение сессий, `Ctrl+C` cancel, `Ctrl+Q` выход. При выборе сессии клиент загружает replay-обновления и дорисовывает текстовую историю.
