...
class TerminalLogModal(ModalScreen[None]):
    ...
    Поддерживает обратную совместимость со старыми методами (add_log, set_logs, clear_logs).
    """
    ...
    def _subscribe_to_view_model(self) -> None:
        ...
        unsub_entries = self.terminal_log_vm.log_entries.subscribe(
    ...
    def add_log(self, entry: str) -> None:
        """Добавить запись в лог (обратная совместимость).
        
        Делегирует вызов методу ViewModel.
        
        Args:
            entry: Текст записи для добавления.
        """
        self.terminal_log_vm.add_entry(entry)
    
    def set_logs(self, entries: list[str]) -> None:
        """Установить все записи лога (обратная совместимость).
        ...
        """
        self.terminal_log_vm.set_entries(entries)
    
    def clear_logs(self) -> None:
        """Очистить все записи (обратная совместимость).
        
        Делегирует вызов методу ViewModel.
        """
        self.terminal_log_vm.clear_entries()
    
    def show_modal(self) -> None:
        """Показать модальное окно (обратная совместимость).
        
        Делегирует вызов методу ViewModel.
        """
        self.terminal_log_vm.show()
