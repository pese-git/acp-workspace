...
class BaseViewModel(ABC):
    ...
    def on_event(self, event_type: Type[DomainEvent], handler: Callable[[DomainEvent], None]) -> None:
        ...
        try:
            # Получаем unsubscribe функцию и сохраняем её для cleanup
            key = f"{event_type.__name__}_{id(handler)}"
