...
| `session/request_permission` | Partial | Базовая server/client оркестрация есть; race с cancel стабилизирован и добавлена persisted policy (`allow_always`/`reject_always`) с scope по tool kind (`other/read/write/execute/search`), остаются demo-ветки. |
