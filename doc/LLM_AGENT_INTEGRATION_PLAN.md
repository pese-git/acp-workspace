...
Разработка разделена на две эпохи в соответствии с архитектурной моделью:
...
├─ Создание абстракций (LLMAgent, LLMProvider)
└─ Инфраструктура для одно-агентного режима

Фаза 1: Базовый одно-агентный режим (NativeAgent + OpenAI)
├─ Реализация NativeAgent
├─ Провайдер OpenAI API
├─ Интеграция с prompt-turn pipeline
└─ Управление state и message history

Фаза 2: Tool Call Generation & Client Integration
...
Фаза 3: Поддержка агентных фреймворков (одно-агентный режим)
├─ Langchain адаптер
├─ Langgraph интеграция
├─ Поддержка других фреймворков
└─ Примеры и документация для фреймворков
```

#### ЭПОХА 2: Мульти-агентный режим (Multi-Agent Mode)

```
Фаза 4: Мульти-агентная система (MultiAgentOrchestrator)
