...
graph TB
    ...
    subgraph ToolsLayer["Слой управления инструментами"]
        ToolRegistry["ToolRegistry<br/>Реестр инструментов"]
        ToolExecutor["ToolExecutor<br/>Выполнитель"]
        PermValidator["PermValidator<br/>Проверка прав"]
    end
    
    subgraph BuiltinTools["Встроенные инструменты"]
    ...
    ToolExecutor --> FSTools
    ToolExecutor --> TermTools
...
classDiagram
    ...
    class ToolRegistry {
        ...
        +execute_tool(session_id, tool_call) ToolExecutionResult
...
graph TB
    ...
    CodeAgent -->|использует| CodeTools
    FSAgent -->|использует| FSTools
    SearchAgent -->|использует| FSTools
    VerifyAgent -->|использует| VerifyTools
...
sequenceDiagram
    ...
    par Parallel Subtask Execution
        ...
        activate SpecAgents
        SpecAgents->>Tools: execute_tool(tool_call)
        activate Tools
        Tools-->>SpecAgents: ToolResult
        deactivate Tools
...
graph TB
    ...
    subgraph ACPServer["ACP Server acp-server"]
        ...
        subgraph Tools["Встроенные инструменты"]
            FileSystem["Файловая система<br/>fs:*"]
            Terminal["Терминал<br/>terminal:*"]
        end
    ...
    subgraph ExternalServices["Внешние сервисы"]
        ...
        Database["База данных<br/>Persistent storage"]
    end
    ...
    SessionMgmt --> Filesystem
    SessionMgmt --> Database
